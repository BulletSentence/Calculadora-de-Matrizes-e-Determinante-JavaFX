...
public class FractalCanvas extends JPanel {
     ...
     * Constructor.
     */
    public FractalCanvas(String initialExpression, JTextField exprField2) {
	iterations = 20;
	nEvals = 0;
	scaleX = 100;
	scaleY = 100;
    ...
    // paintRegion(g, x + width/2, y, width - width/2, height/2, depth+1,
    // depth_max);
    // paintRegion(g, x, y + height/2, width/2, height - height/2, depth+1,
    // depth_max);
    // paintRegion(g, x + width/2, y + height/2, width - width/2, height -
    // height/2, depth+1, depth_max);
    // }
    // }
	...
	if (!hasError) {
	    ...
	    System.out.printf("done. secs: %6.3f microsec/eval: %6.3f%n", seconds, 1000000 * seconds / nEvals);
	}
	/*
	 * if (!initializedBuffer) { buffer = createImage(dimensions.width,
	 * dimensions.height); initializedBuffer = true; }
	 * 
	 * Graphics buffergc = buffer.getGraphics();
	 * 
	 * if (changedFunction) { paintWhite(buffergc); if (!hasError)
	 * paintFractal(buffergc); g.drawImage(buffer, 0, 0, null);
	 * changedFunction = false; } else { g.drawImage(buffer, 0, 0, null); }
	 */
    }
}
